...
3. Lever:
   - Har du BARE endret datafiler – src/lib/participants.ts, src/lib/schedule.ts, src/lib/info.ts, src/lib/packing.ts, src/lib/river.ts, src/lib/places.ts, src/lib/config.ts, src/lib/runs.ts, src/lib/rooms.ts, eller filer under static/bilder – commit og push rett til standardgrenen.
...
- Siden er skjult og avsenderen («Fra: <navn>») er en deltaker. Ønsket kan gjelde avsenderen selv eller andre («Caroline sa hun ikke vil padle Playrun», «Bytt Helene og Caroline til gruppe 3»). Gjør det, og si i kommentaren hvem det kom via.
...
- Erfarne som vil stå over Playrun på et run settes i `noPlayrun` på det runnet (de går i land med rookiene og sitter på i 9-seteren), og `maybePlayrun` for de som velger selv. Oppdater tekstene i `steps` som nevner dem.
...
- Planen må fortsatt gå opp, og det er ditt ansvar å sjekke: 21 padler (12 erfarne, 9 rookies: 5 på run 1, 4 på run 2); 18 kajakker med (12 erfarne har hver sin, rookiene deler på 5, den 18. er reserve); kajakkplass henger 10, Wiktor 2 (kan ta 4 om det må), Tiril 4 (takstativ), Adrian 4, Caroline 2, altså 22; om morgenen går 10 på hengeren, 3 på Tirils bil, 2 på Wiktors, 2 på Carolines og Adrians egen på hans; Adrian er bare med lørdag, står ikke i src/lib/participants.ts, og kjører bare sin egen bil; Tiril dro hjem, men bilen hennes er med (automat, Helene og Ludvig kjører den, Maja kan også); før hvert run settes Wiktors, Carolines og Adrians bil på take out Playrun så alle tre kjører hjem derfra, og Tirils bil på take out Bru-bru så den tar rookiene videre med 4 kajakker, den femte går på hengeren; `shortPlan` øverst på /runs er kortversjonen som leses ved frokosten, hold den i takt med resten; hvert kjørelegg kan ha `alt` med andre som like godt kan kjøre, og `parked: true` betyr at bilen står; seter 9-seteren 9, Wiktor 5, Tiril 5, Helene 4, Caroline 3, og Helenes bil står på Kruke hele dagen; førerkort som i src/lib/participants.ts (automat kan bare kjøre Carolines og Tirils bil; ingen uten oppgitt lapp kjører); 9-seteren med henger kjøres bare av noen med manuell lapp; hvert run må ha nok sjåfører til å få alle biler og kajakker dit de skal; ingen skal kjøre noe de har sagt de ikke vil kjøre; ingen rookie padler to runs uten å ha bedt om det. Går det ikke opp, gjør det som går opp og forklar resten i kommentaren, eller avvis.
...
- Soveplasser: ønsker om rom («Jeg vil sove i hytta», «Jeg og Maren vil dele rom») gjelder src/lib/rooms.ts. Legg navnet i `taken` på rommet hvis det er plass (dobbeltseng teller to), ellers i `wish`, og si fra i kommentaren. Ikke flytt noen som allerede har fått plass uten at de ber om det selv.
